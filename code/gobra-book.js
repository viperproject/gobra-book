...
function language_of(block) {
    ...
    if (languages.length == 0) {
        console.warn("no language detected for code block. using default " + language_default);
...
(function (editors) {
    ...
    let code_nodes = Array.from(document.querySelectorAll("code")).filter((n) => n.parentElement.tagName == "PRE");
    ...
    return (editor) => {
        ...
        lines.forEach((line, line_number) => {
            ...
            if (match) {
                ...
                markers.push(session.addMarker(new Range(line_number, match.index, line_number, line.length + 1), "errorHighlight", "background"));
            }
            Array(...line.matchAll(GOBRA_INLINE)).forEach((match) => {
                let start = match.index;
                let end = start + match[0].length;
                console.debug("Found gobra annotation: ", match[0], line_number, start, end);
                markers.push(session.addMarker(new Range(line_number, start, line_number, end), "errorHighlight", "background"));
            });
        });
    };
}
